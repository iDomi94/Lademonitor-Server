...
def attach_consumption(db: Session, sessions: list[models.ChargingSession]) -> None:
...
def resolve_location(session: models.ChargingSession, db: Session) -> None:
    """Matcht einen bekannten Ladeort per GPS, sonst Fallback auf Reverse-Geocoding."""
    if session.latitude is None or session.longitude is None:
        return
    matched = match_location(db, session.latitude, session.longitude)
    if matched:
...
def list_sessions(
    ...
    attach_consumption(db, sessions)
    return sessions
...
def create_session(payload: schemas.SessionCreate, db: Session = Depends(get_db)):
    ...
    session = models.ChargingSession(**payload.model_dump(), source=models.SessionSource.MANUAL)
    if not session.location_id:
        resolve_location(session, db)
    ...
    db.commit()
    db.refresh(session)
    attach_consumption(db, [session])
    return session


@router.patch("/{session_id}", response_model=schemas.SessionOut)
def update_session(session_id: str, payload: schemas.SessionUpdate, db: Session = Depends(get_db)):
    session = db.get(models.ChargingSession, session_id)
    if not session:
...
def push_auto_session(payload: schemas.AutoSessionPush, db: Session = Depends(get_db)):
    """Wird von der Home-Assistant-Automation nach Ladeende aufgerufen."""
    vehicle = (
        db.query(models.Vehicle)
        .filter(models.Vehicle.external_id == payload.vehicle_external_id)
        .first()
    )
    if not vehicle:
        raise HTTPException(404, f"Kein Vehicle mit external_id '{payload.vehicle_external_id}'")

    # Duplikat-Schutz: gleiche external_session_id nicht zweimal anlegen
    existing = (
        db.query(models.ChargingSession)
        .filter(models.ChargingSession.external_session_id == payload.external_session_id)
        .first()
    )
    if existing:
        attach_consumption(db, [existing])
        return existing
