...
CHANGELOG = [
    ...
    },
    {
        ...
        "changes": [
            "Neu, optional: GPS-Koordinaten (Ladeorte und Ladevorgänge, auch ein gerade laufender MyŠkoda-Ladevorgang), Notizen (Ladevorgänge UND Anbieter), automatisch ermittelte Ortsnamen, Fahrzeug-/Ladeort-Namen, die Fahrzeug-Identifizierungsnummer (VIN), das MyŠkoda-Debug-Protokoll (komplette Rohantworten der API) sowie die gespeicherten Zugangsdaten (SMTP-Passwort, WebDAV-Adresse/-Nutzername/-Passwort, MyŠkoda-API-Key) können jetzt verschlüsselt statt im Klartext in der Datenbank gespeichert werden - relevant, sobald der Server öffentlich erreichbar ist (z.B. über einen eigenen Reverse Proxy). Standardmäßig AUS, wer nur im eigenen Heimnetz unterwegs ist, braucht nichts zu tun.",
            "Bei einer eigenen DSGVO-Prüfung durch systematisches Durchgehen aller Datenbankfelder gefunden und mitkorrigiert: der MyŠkoda-Poller baute die interne Dubletten-Kennung eines Ladevorgangs bisher aus der VIN (`myskoda-<VIN>-<Startzeit>`) - die wäre trotz verschlüsselter VIN-Spalte über dieses zweite, für den Dublettencheck zwingend unverschlüsselte Feld wieder im Klartext gelandet. Läuft jetzt über die interne, nicht-personenbezogene Fahrzeug-ID statt der VIN.",
            ...
            "Bewusst NICHT verschlüsselt, weil an SQL-Gleichheitsvergleiche bzw. Eindeutigkeits-Prüfungen pro Nutzer gebunden (eine Verschlüsselung würde diese Prüfungen unbemerkt wirkungslos machen): Fahrzeug-Kurzschlüssel (`external_id`, wird beim Home-Assistant-Push exakt abgefragt), Anbieter-Name, Nutzername und E-Mail-Adresse. Der eingebaute CSV-Export und das automatische WebDAV-Backup liefern alle verschlüsselten Werte weiterhin bewusst im Klartext, da sie ein portables, menschenlesbares Format bleiben sollen.",
        ],
    },
    {
        "version": "0.14.1",
