...
CHANGELOG = [
    {
        ...
        "changes": [
            "Neu, optional: GPS-Koordinaten (Ladeorte und Ladevorgänge), Notizen, automatisch ermittelte Ortsnamen, Fahrzeug-/Ladeort-Namen sowie die gespeicherten Zugangsdaten (SMTP-Passwort, WebDAV-Passwort, MyŠkoda-API-Key) können jetzt verschlüsselt statt im Klartext in der Datenbank gespeichert werden - relevant, sobald der Server öffentlich erreichbar ist (z.B. über einen eigenen Reverse Proxy). Standardmäßig AUS, wer nur im eigenen Heimnetz unterwegs ist, braucht nichts zu tun.",
